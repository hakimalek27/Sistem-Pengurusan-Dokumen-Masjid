...
#
# W4 mengubah `resources/js/help.js` -> nama aset BERUBAH
# (help-D0185fq1.js -> help-DaHF3IsK.js; help-CrH0eDM1.css KEKAL), jadi rebuild
# `app` DAN `nginx` adalah WAJIB. `catalog_version` berubah -> sync-help-index --delete.
# ═══════════════════════════════════════════════════════════════════════════════════════
set -euo pipefail

SHA="${1:?guna: deploy-10.sh <git-sha-penuh>}"
cd /opt/diwan
