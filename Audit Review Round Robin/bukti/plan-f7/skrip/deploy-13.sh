#!/usr/bin/env bash
# ═══════════════════════════════════════════════════════════════════════════════════════
# DEPLOY 13 — F7 (A11y) ke bakwim.my
#
...
#
# F7 menyentuh help.js, document-viewer.js, DAN memperkenalkan entri Vite BAHARU
# (a11y-landmarks.js). Jadi ramalannya BERBEZA daripada Deploy 12 (yang nama asetnya kekal):
#
#   help-D_qumira.js          -> assets/help-EPOANIj9.js          (BERUBAH: help.js disunting)
#   help-Cfwb6f_j.css         -> assets/help-Cfwb6f_j.css         (KEKAL: help.css tidak disentuh)
#   (tiada)                   -> assets/a11y-landmarks-mQ2zo0LK.js (BAHARU — entri keenam Vite)
#   document-viewer-*.js      -> assets/document-viewer-sF29a_1w.js
#
# `catalog_version` KEKAL 2026.08.08.1 — F7 tidak menyentuh katalog. `sync-help-index`
# dikekalkan dalam skrip kerana ia idempoten, tetapi ia BUKAN bukti apa-apa kali ini.
# Rebuild `app` DAN `nginx` WAJIB (aset berubah + satu aset BAHARU mesti wujud dalam imej web).
# ⛔ JANGAN jalankan seeder pada produksi.
#
# Nilai jangkaan #2a selepas deploy (tidak berubah — itu SENDIRI ialah semakan):
#   catalog_version 2026.08.08.1 | guide 83 | langkah 473 | generik 59
# ═══════════════════════════════════════════════════════════════════════════════════════
set -euo pipefail

SHA="${1:?guna: deploy-13.sh <git-sha-penuh>}"
cd /opt/diwan
...
foreach($m as $k=>$v){ if(strpos($k,"help")!==false||strpos($k,"a11y")!==false||strpos($k,"document-viewer")!==false){ echo $k." -> ".$v["file"]; if(!empty($v["css"])) echo "  css: ".implode(",",$v["css"]); echo PHP_EOL; } }' < /dev/null
...
echo "--- #3c aset a11y BAHARU mesti wujud dalam imej app (entri Vite keenam) ---"
sudo docker compose exec -T app sh -lc 'ls -1 public/build/assets/ | grep "^a11y-landmarks-" || echo "TIADA — vite.config.js tidak dikemas!"' < /dev/null
