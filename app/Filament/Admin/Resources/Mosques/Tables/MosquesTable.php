        ...
        return $table
            ...
            ->recordActions([
                ...
                DeleteAction::make()
                    ...
                    ->after(fn ($record) => activity()->performedOn($record)->causedBy(Auth::user())->log('tenant_diarkibkan')),
                RestoreAction::make()
                    ->label('Pulihkan')
                    ->after(fn ($record) => activity()->performedOn($record)->causedBy(Auth::user())->log('tenant_dipulihkan')),

                Action::make('lulus')
                    ->label('Lulus')->color('success')->icon('heroicon-o-check')
                    ->authorize(fn () => Auth::user()?->is_superadmin ?? false)
                    ->visible(fn ($record) => ! $record->trashed() && $record->status === MosqueStatus::Menunggu)
                ...
                Action::make('gantung')
                    ...
                    ->authorize(fn () => Auth::user()?->is_superadmin ?? false)
                    ->visible(fn ($record) => ! $record->trashed() && in_array($record->status, [MosqueStatus::Aktif, MosqueStatus::Digantung], true))
                    ->requiresConfirmation()
                    ->action(function ($record) {
                        ...
                        activity()->performedOn($record)->causedBy(Auth::user())
                            ->withProperties(['status' => $new->value, 'ip' => request()->ip()])->log('ubah_status_tenant');
                ...
                Action::make('masukPanel')
                    ...
                    ->authorize(fn () => Auth::user()?->is_superadmin ?? false)
                    ->visible(fn ($record) => ! $record->trashed() && $record->status === MosqueStatus::Aktif)
